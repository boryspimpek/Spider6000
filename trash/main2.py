...
GAIT_CONFIGS = {
    ...
    GaitMode.TURN_RIGHT: GaitParams(
        x_amps=(-30, -30, -30, -30),
        z_amps=(15, -15, -15, 15),
    ...
    GaitMode.TROT: GaitParams(
        x_amps=(30, 30, 30, 30),
        z_amps=(15, 15, 15, 15),
        x_offsets=(85, 95, 45, 135),
        z_offsets=(90, 90, 90, 90),
        phase_offsets=(0.00, 0.50, 0.00, 0.50)  # LF i LR w fazie, RF i RR w fazie
    )
}
...
def creep_gait(x_amp, z_amp, x_off, z_off, phase):
    ...
    # RETURN (25-100% cyklu) 
        ...
        x = x_off + x_amp * (1 - normalized_return_phase)  # liniowy powrót    return z, x
    return z, x
...
def print_gait_info(step, t, angles, mode):
    """Wyświetla informacje o aktualnym stanie chodu w formie ciągłej"""
...
def execute_gait(mode):
    ...
    try:
        while True:
            ...
            servo_mapping = [
                (1, angles['lf']['x']), (2, angles['lf']['z']),
                (3, angles['rf']['x']), (4, angles['rf']['z']),
                (5, angles['lr']['x']), (6, angles['lr']['z']),
                (7, angles['rr']['x']), (8, angles['rr']['z'])
            ]
            
            for servo_id, angle in servo_mapping:
...
if __name__ == "__main__":
    ...
    # execute_gait(GaitMode.TURN_LEFT)
    execute_gait(GaitMode.TURN_RIGHT)
